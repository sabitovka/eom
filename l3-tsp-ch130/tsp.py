...
class TravelingSalesmanProblem:
  ...
  def __initData(self):
    ...
    try:
      self.locations = pickle.load(open(os.path.join("tsp-data", \
        self.name + "-loc.pickle"), "rb"))
      self.distances = pickle.load(open(os.path.join("tsp-data", \
        self.name + "-dist.pickle"), "rb"))
  ...
  def __createData(self):
    ...
    with urlopen("http://elib.zib.de/pub/mp-testdata/tsp/tsplib/tsp/" \
      + self.name + ".tsp") as f:
      ...
      for i in range(self.tspSize):
        for j in range(i + 1, self.tspSize):
          ...
          print("{}, {}: location1 = {}, location2 = {} => distance = {}" \
            .format(i, j, self.locations[i], self.locations[j], distance))

      # serialize locations and distances:
      if not os.path.exists("tsp-data"):
        os.makedirs("tsp-data")
        pickle.dump(self.locations, open(os.path.join("tsp-data", \
          self.name + "-loc.pickle"), "wb"))
        pickle.dump(self.distances, open(os.path.join("tsp-data", \
          self.name + "-dist.pickle"), "wb"))
...
def main():
  ...
  optimalSolution = [0, 27, 5, 11, 8, 25, 2, 28, 4, 20, 1, 19, 9, 3,
    14, 17, 13, 16, 21, 10, 18, 24, 6, 22, 7, 26, 15, 12, 23]
